...
import type { ReadonlyJSONValue } from "../zero-unexposed/json.ts";
import type { FrozenJSONValue } from "../zero-unexposed/frozen-json.ts";
import { isDeepFrozen } from "../zero-unexposed/frozen-json.ts";
import {
  withRead,
  withWrite,
  withWriteNoImplicitCommit,
} from "./with-transactions.ts";
import type { Read, Store, Write } from "../zero-unexposed/store.ts";
...
async function store(store: TestStore): Promise<void> {
  ...
  expect(await store.has("foo")).toBe(true);
  expect(await store.get("foo")).toEqual("baz");
  ...
  expect(await store.get("baz")).toEqual("bat");
}
...
async function readTransaction(store: TestStore): Promise<void> {
  ...
  await withRead(store, async (rt) => {
    ...
    expect("v1").toEqual(await rt.get("k1"));
  });
}
...
async function writeTransaction(store: TestStore): Promise<void> {
  ...
  expect(await store.get("k1")).toEqual("overwrite");
  expect(await store.get("k2")).toEqual("v2");
  ...
  expect(await store.get("k2")).toEqual("final");

  // Test Read interface on Write.
  await withWrite(store, async (wt) => {
    await wt.put("k2", "new value");
    expect(await wt.has("k2")).toBe(true);
    expect(await wt.get("k2")).toEqual("new value");
  });
}
...
async function isolation(store: Store): Promise<void> {
  ...
  const w2 = store.write().then(async (w) => {
    ...
    expect(await w.get("k1")).toEqual("w1");
  ...
  const r3 = store.read().then(async (r) => {
    ...
    expect(await r.get("k1")).toEqual("w2");
    log.push("r3 end");
    r.release();
  });
...
async function writeTransactionRollbackOnError(
  ...
  await withRead(store, async (rt) => {
    ...
    expect(await rt.get("existing")).toEqual("value"); // Original value
  });
}
...
async function readAndWriteTransactionStateManagement(
  ...
  await withWrite(store, async (wt) => {
    ...
    expect(await wt.get("test-key")).toEqual("test-value");
  });

  // Verify the write was committed
  await withRead(store, async (rt) => {
    expect(await rt.get("test-key")).toEqual("test-value");
  });
}
...
async function nullAndUndefinedValueHandling(store: TestStore): Promise<void> {
  ...
  await withRead(store, async (rt) => {
    ...
    expect(await rt.get("empty-string-key")).toEqual("");
...
async function multipleStoresWithSameNameShareData(
  ...
  expect(await store.get("shared")).toEqual("data");
}
...
async function multipleStoresWithDifferentNamesHaveSeparateData(
  ...
  expect(await store.get("key")).toEqual("value1");
}
...
async function errorHandlingForClosedTransactions(
  ...
  expect(readTx.has("key")).rejects.toThrow("Transaction is closed");
  expect(readTx.get("key")).rejects.toThrow("Transaction is closed");

  // Test write transaction error handling
  const writeTx = await store.write();
  writeTx.release();

  expect(writeTx.has("key")).rejects.toThrow("Transaction is closed");
  expect(writeTx.get("key")).rejects.toThrow("Transaction is closed");
  expect(writeTx.put("key", "value")).rejects.toThrow("Transaction is closed");
  expect(writeTx.del("key")).rejects.toThrow("Transaction is closed");
  expect(writeTx.commit()).rejects.toThrow("Transaction is closed");
}
...
async function errorHandlingForClosedStore(store: TestStore): Promise<void> {
  ...
  expect(store.write()).rejects.toThrow();
}
